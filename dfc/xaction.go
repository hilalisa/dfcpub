	...
	if xx != nil {
		...
		if !xreb.Finished() {
			if xreb.curversion > curversion {
				glog.Errorf("%s version is greater than curversion %d", xreb.String(), curversion)
				q.lock.Unlock()
				return nil
			}
			if xreb.curversion == curversion {
				glog.Infof("%s already running, nothing to do", xreb.String())
				q.lock.Unlock()
				return nil
			}
			xreb.abort()
...
func (q *xactInProgress) renewLRU(t *targetrunner) *xactLRU {
	...
	if xx != nil {
		xlru := xx.(*xactLRU)
		glog.Infof("%s already running, nothing to do", xlru.String())
		q.lock.Unlock()
		return nil
	}
	id := q.uniqueid()
...
func (q *xactInProgress) renewElection(p *proxyrunner, vr *VoteRecord) *xactElection {
	...
	if xx != nil {
		...
		glog.Infof("%s already running, nothing to do", xele.String())
		q.lock.Unlock()
		return nil
	}
	id := q.uniqueid()
...
func (q *xactInProgress) renewRechecksum(t *targetrunner, bucket string) *xactRechecksum {
	...
	for _, xx := range q.findUAll(cmn.ActRechecksum) {
		...
		if xrcksum.bucket == bucket {
			glog.Infof("%s already running for bucket %s, nothing to do", xrcksum.String(), bucket)
			return nil
		}
	}
	id := q.uniqueid()
